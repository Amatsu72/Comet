...
#define CHECK_VK(func) checkVkResult(func, __FILE_NAME__, __LINE__, #func)
...
static const char *vkResultToString(VkResult code)
{
    switch (code) {
...
static void checkVkResult(VkResult result, const char *fileName, uint32_t line, const char* func){
    if(result != VK_SUCCESS){
        LOG_ERROR("[{0}:{1}]: {2} ---> {3}", fileName, line, func, vkResultToString(result));
    }
}